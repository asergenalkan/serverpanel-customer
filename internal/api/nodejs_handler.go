...
func (h *Handler) createApacheProxyConfig(appID int64, port int, appURL string) {
	...
	// Create proxy config snippet
	configContent := fmt.Sprintf(`
# Node.js App %d Proxy Config
<Location %s>
    ProxyPass http://127.0.0.1:%d%s
    ProxyPassReverse http://127.0.0.1:%d%s
    ProxyPreserveHost On
</Location>
`, appID, path, port, path, port, path)

	configPath := fmt.Sprintf("/etc/apache2/conf-available/nodejs-app-%d.conf", appID)
	os.WriteFile(configPath, []byte(configContent), 0644)

	// Enable config and reload Apache
	exec.Command("a2enconf", fmt.Sprintf("nodejs-app-%d", appID)).Run()
	exec.Command("systemctl", "reload", "apache2").Run()

	// Also add to domain's vhost if exists
	...
	if _, err := os.Stat(vhostPath); err == nil {
		...
		if !strings.Contains(string(content), fmt.Sprintf("nodejs-app-%d", appID)) {
			// Add include before </VirtualHost>
			includeDirective := fmt.Sprintf("\n    Include conf-available/nodejs-app-%d.conf\n", appID)
			newContent := strings.Replace(string(content), "</VirtualHost>", includeDirective+"</VirtualHost>", 1)
			os.WriteFile(vhostPath, []byte(newContent), 0644)
			exec.Command("systemctl", "reload", "apache2").Run()
		}
	}
}
